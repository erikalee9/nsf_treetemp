...
full_pers_db_plots <- ggarrange(feb_pers_db_plot, march_pers_db_plot, april_pers_db_plot, may_pers_db_plot, nrow = 2, ncol = 2)
...
feb_trans_db_plot <- ggplot(feb_trans_db) + 
  geom_boxplot(aes(x = aspect, y = db_temp, fill = aspect), alpha = 0.7) + 
  facet_wrap(~ zone + burn_status + month) + 
  scale_fill_manual(
    values = c("north" = "#2166AC", "south" = "#D95F02"),
    name = "Aspect of Temp Sensor"  # Legend title
...
```{r}
#combining individual month PSZ plots
full_trans_db_plots <- ggarrange(feb_trans_db_plot, march_trans_db_plot, april_trans_db_plot, may_trans_db_plot, nrow = 2, ncol = 2)
...
full_db_plots <- ggarrange(full_pers_db_plots, full_trans_db_plots, nrow = 1, ncol = 2)
...
\*\* Instead, I could do patterened plots by snow zone and have both snow zones on each months plot? Would that make more sense?
