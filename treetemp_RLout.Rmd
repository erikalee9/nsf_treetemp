...
```{r}
#plotting gb/db/ub and north/south comparisons!
accu_dt_gbdbub_plot <- ggplot(data = accu_dt_gbdbub_LWout) +
  geom_line(aes(x = day, y = accu_daily_gb_LWout_MJperd, color = "green burn")) +  # First line labeled "No Snow"
  geom_line(aes(x = day, y = accu_daily_db_LWout_MJperd, color = "dead burn")) +
  geom_line(aes(x = day, y = accu_daily_ub_LWout_MJperd, color = "live")) +
  ...
  labs(
    ...
    x = "Date",
    y = "Daily Accumulated LWout (MJ/day)",
    color = "Burn Status"  # Legend title
  ) +
