...
```{r}
#don't think I use this dataframe, but here to load if needed
...
pairwise_diffs_ind <- expand_grid(var1 = temp_varsdiff_1, var2 = temp_varsdiff_2) %>%
  ...
  rowwise() %>%
  mutate(
    mean_diff = mean_avg_diff(PSZ_comb_ind_alltrees[[var1]], PSZ_comb_ind_alltrees[[var2]]),
    sd_diff = sd(PSZ_comb_ind_alltrees[[var1]] - PSZ_comb_ind_alltrees[[var2]])
  ) %>%
  ungroup()
...
```{r}
# Visualizing the mean absolute difference as a heatmap with values inside
PSZ_ind_heatmap_sd <- ggplot(pairwise_diffs_ind, aes(x = var1, y = var2, fill = sd_diff)) +
  geom_tile() +  # Create heatmap tiles
  geom_text(aes(label = sprintf("%.1f", sd_diff)), color = "black", size = 4) +  # Add values in squares
  scale_fill_gradient2(low = "white", mid = "orangered3", high = "red4", 
                       midpoint = 6,
                       limits = c(0, 12)) + 
  theme_minimal() +
...
Plotting the standard deviation as well, to get stats

```{r}
# Visualizing the mean standard difference as a heatmap with values inside
TSZ_ind_heatmap_sd <- ggplot(TSZ_pairwise_diffs_ind, aes(x = var1, y = var2, fill = sd_diff)) +
  geom_tile() +  # Create heatmap tiles
  geom_text(aes(label = sprintf("%.1f", sd_diff)), color = "black", size = 4) +  # Add values in squares
  scale_fill_gradient2(low = "white", mid = "orangered3", high = "red4", 
                       midpoint = 6,
...
Exploring when to define "ablation" as starting

### V1

-   not using this snotel data, as it's from multiple different locations that do not well-represent our sitres
...
-   using this data for thesis!

<!-- -->

-   Joe Wright and Hourglass Lake SNOTEL sites to get predicted snow density, then applying to our snow depth data
...
-   Single mean value per-day of daytime tree temp, air temp and net SW
...
-   Single mean value per-day of nighttime tree temp, air temp and net SW
