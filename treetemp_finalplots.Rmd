...
\*\* see if I actually need to separate into tree 1 and tree 2, or if I can just make that distinction in function, since I just want to compare between tree 1 and tree 2!
...
pairwise_diffs <- expand_grid(var1 = temp_vars, var2 = temp_vars) %>%
  filter(var1 != var2) %>%  # Remove self-comparisons
  rowwise() %>%
  mutate(mean_diff = mean_avg_diff(comb_full_ns_data[[var1]], comb_full_ns_data[[var2]])) %>%
  ungroup()
...
# Add 'phase' and 'zone' to the pairwise_diffs dataframe by joining with comb_full_ns_data
pairwise_diffs <- pairwise_diffs %>%
  left_join(comb_full_ns_data %>% select(datetime, phase, zone) %>% distinct(), 
            by = c("datetime" = "datetime"))

# Visualizing the mean absolute difference as a heatmap with values inside
ggplot(pairwise_diffs, aes(x = var1, y = var2, fill = mean_diff)) +
  geom_tile() +  # Create heatmap tiles
  geom_text(aes(label = round(mean_diff, 2)), color = "black", size = 5) +  # Add values in squares
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                       midpoint = mean(pairwise_diffs$mean_diff, na.rm = TRUE)) +
  theme_minimal() +
