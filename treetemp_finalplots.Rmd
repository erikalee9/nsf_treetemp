...
march_dt_mean_gb_temp_plot <- ggplot(data = march_dt_mean_gb_temps) +
  ...
  facet_wrap(~zone) +
  labs(title = "a) Accumulation", x = "Date", y = "Temperature (°C)") +
  scale_color_manual(
    name = "aspect",
...
p_snowtreewell_NR_plot <- ggplot() + 
  ...
  geom_col(data = mean_dt_persb_wx, aes(x = day, y = NR_Avg_MJperd, fill = "net radiation"), position = "stack", alpha = 0.7) +
  #air temp line
  ...
  #transitional snow depths + tree wells
  geom_line(data = persb_snowcamera_data, aes(x = date, y = snowcam_max_depth_m*5, color = "daily max snow depth"), linewidth = 0.8) +
  geom_point(data = persb_treewell_dates, aes(x = date, y = snowcam_max_depth_m*5, 
             shape = "dates that treewells developed"), 
             color = "black", fill = "black", size = 1.5) +
  ...
  labs(
    title = "a) PSZ",
    x = NULL,
    y = "Total Net Rad (MJ/day) & Air Temp (°C)"
  ) +
  # Customizing colors and shapes
  scale_color_manual(name = "Radiation Data", 
                     values = c("air temp" = "#D95F02", "daily max snow depth" = "#2166AC")) +scale_fill_manual(name = "Radiation Data", 
...
t_snowtreewell_NR_plot <- ggplot() + 
  # Column plot for Net Longwave Radiation of total plot area
  geom_col(data = mean_dt_transb_wx, aes(x = day, y = NR_Avg_MJperd, fill = "net radiation"), position = "stack", alpha = 0.7) +
  #air temp line
  ...
  geom_point(data = transb_treewell_dates, aes(x = date, y = snowcam_max_depth_m*5, 
             shape = "dates that treewells developed"), 
             color = "black", fill = "black", size = 1.5) +
  facet_wrap(~burn_status) +
  labs(
  ...
  # Customizing colors and shapes
  scale_color_manual(name = "Radiation Data", 
                     values = c("air temp" = "#D95F02", "daily max snow depth" = "#2166AC")) +
