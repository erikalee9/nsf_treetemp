...
PSZ_ind_heatmap <- ggplot(pairwise_diffs_ind, aes(x = var1, y = var2, fill = mean_diff)) +
  ...
  scale_fill_gradient2(low = "blue2", mid = "white", high = "orangered3", 
                       midpoint = 4,
                       limits = color_limits) +
  theme_minimal() +
  labs(title = "a) PSZ",
...
#using this!
ggsave(
  filename = "comb_ind_heatmap_V2.png",  # File name with extension
...
ggsave(
  filename = "full_comb_heatmap.png",  # File name with extension
...
final_daynight_comb <- full_day %>%
  inner_join(full_night, by = c("day", "zone", "snow_phase"))
```

```{r}
PSZ_daynight <- final_daynight_comb %>%
  filter(zone == "PSZ")

TSZ_daynight <- final_daynight_comb %>%
  filter(zone == "TSZ")
```
...
pairwise_diffs_PSZ_dtnt <- expand_grid(var1 = temp_vars, var2 = temp_vars) %>%
  rowwise() %>%
  mutate(mean_diff = mean_avg_diff(PSZ_daynight[[var1]], PSZ_daynight[[var2]])) %>%
  ungroup()

#setting a limit for all four plots that is incorporated into each plot, so the color gradient is equal across all plots
color_limits <- c(0, 12)

diffs_PSZ_dtnt_heatmap <- ggplot(pairwise_diffs_PSZ_dtnt, aes(x = var1, y = var2, fill = mean_diff)) +
  geom_tile() +  # Create heatmap tiles
  geom_text(aes(label = round(mean_diff, 1)), color = "black", size = 4) +  # Add values in squares
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                       midpoint = 4,
...
pairwise_diffs_PSZ_dtnt_V2 <- expand_grid(var_day = day_vars, var_night = night_vars) %>%
  rowwise() %>%
  mutate(mean_diff = mean_avg_diff(PSZ_daynight[[var_day]], PSZ_daynight[[var_night]])) %>%
  ungroup()

# Plot: x = night temps, y = day temps
diffs_PSZ_dtnt_heatmap_V2 <- ggplot(pairwise_diffs_PSZ_dtnt_V2, aes(x = var_night, y = var_day, fill = mean_diff)) +
  geom_tile() +  
  geom_text(aes(label = round(mean_diff, 1)), color = "black", size = 4) +  
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                       midpoint = 4,
...
# Only compare day (y-axis) vs. night (x-axis)
pairwise_diffs_TSZ_dtnt_V2 <- expand_grid(var_day = day_vars, var_night = night_vars) %>%
  rowwise() %>%
  mutate(mean_diff = mean_avg_diff(TSZ_daynight[[var_day]], TSZ_daynight[[var_night]])) %>%
  ungroup()

# Plot: x = night temps, y = day temps
diffs_TSZ_dtnt_heatmap_V2 <- ggplot(pairwise_diffs_TSZ_dtnt_V2, aes(x = var_night, y = var_day, fill = mean_diff)) +
  geom_tile() +  
  geom_text(aes(label = round(mean_diff, 1)), color = "black", size = 4) +  
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                       midpoint = 4,
...
# Print the plot
diffs_TSZ_dtnt_heatmap_V2
```

```{r}
comb_dtnt_heatmap_V2 <- diffs_PSZ_dtnt_heatmap_V2 + diffs_TSZ_dtnt_heatmap_V2 +
  plot_layout(ncol = 2, guides = "collect") &
  theme(legend.position = "right")
