...
wide_dt_gbdbub_temps <- dt_full_daily_gbdbub_plot_dataframe %>%
  pivot_wider(names_from = phase, values_from = c(gb_temp, db_temp, ub_temp))

wide_nt_gbdbub_temps <- nt_full_daily_gbdbub_plot_dataframe %>%
  pivot_wider(names_from = phase, values_from = c(gb_temp, db_temp, ub_temp))

wide_dtnt_gbdbub_temps <- wide_dt_gbdbub_temps %>%
  inner_join(wide_nt_gbdbub_temps)
```
...
stats_totaltemprange_gbdbub <- full_15min_gbdbub %>%
  group_by(zone, phase, snow_phase, aspect) %>%
  filter(phase == "day") %>%
  summarize(
    mean_gb_temp = mean(gb_temp, na.rm = TRUE), 
    mean_db_temp = mean(db_temp, na.rm = TRUE), 
