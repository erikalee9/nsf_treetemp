...
PSZ_threeday_accu_comp_lineplot <- ggplot(data = PSZ_threeday_accu_comp) + 
  ...
  scale_x_datetime(
    ...
    labels = c("Feb 10th", "Feb 11th", "Feb 12th")
  ) +
  scale_color_manual(
    name = NULL,
...
### Plotting Matrix - NOT WORKING

```{r}
data <- read.csv("your_file.csv", row.names = 1)
data_long <- as.data.frame(as.table(as.matrix(data)))
...
```{r}
# Step 1: Load the data (assuming the first column contains row names)
data_matrix <- as.matrix(read.csv("individualsensor_temp_matrix_data.csv"))
...
TSZ_dtnt_treetemp_daily_gbdbub_lineplot <- ggplot(data = TSZ_dtnt_daily_gbdbub_plot_dataframe ) +
  ...
  geom_vline(xintercept = as.numeric(as.Date('2024-04-01')), 
             ...
             size = 0.4, alpha = 0.7) +
  #snowfree date
  geom_vline(xintercept = as.numeric(as.Date('2024-05-08')), 
             linetype = "dashed",                # Dashed line
             color = "grey23",                      # Line color
             size = 0.4, alpha = 0.8) +
   theme(
...
#use this plot!
ggsave(
  filename = "PSZ_meandaily_ns_lineplot.png",  # File name
...
```{r}
ggsave(
  filename = "TSZ_meandaily_ns_lineplot.png",  # File name
...
```{r}
ggsave(filename = "final_comb_snowdepth_lineplot_V4.png", 
...
```{r}
ggsave(filename = "final_comb_snowdepth_barplot_V2.png", 
...
```{r}
PSZ_ind_sensor_tree1_n <- PSZ_threeday_accu_comp %>%
...
```{r}
PSZ_ind_sensor_tree1_s <- PSZ_threeday_accu_comp %>%
  select(-c(solarNoon, sunrise, sunset, dawn, dusk)) %>%
...
```{r}
PSZ_ind_sensor_tree2_n <- PSZ_threeday_accu_comp %>%
  select(-c(solarNoon, sunrise, sunset, dawn, dusk)) %>%
...
```{r}
#south tree 2 
PSZ_ind_sensor_tree2_s <- PSZ_threeday_accu_comp %>%
  select(-c(solarNoon, sunrise, sunset, dawn, dusk)) %>%
...
rename_vars_ind <- c(
  "tree1_DB_north" = "DB North",
  "tree1_DB_south" = "DB South",
  "tree1_GB_north" = "GB North",
  "tree1_GB_south" = "GB South",
  "tree1_live_north" = "Live North",
  "tree1_live_south" = "Live South", 
  "tree2_DB_north" = "DB North",
  "tree2_DB_south" = "DB South",
  "tree2_GB_north" = "GB North",
  "tree2_GB_south" = "GB South",
  "tree2_live_north" = "Live North",
  "tree2_live_south" = "Live South"
)
...
ggplot(pairwise_diffs_ind, aes(x = var1, y = var2, fill = mean_diff)) +
  geom_tile() +  # Create heatmap tiles
  geom_text(aes(label = round(mean_diff, 2)), color = "black", size = 5) +  # Add values in squares
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                       midpoint = mean(pairwise_diffs_ind$mean_diff, na.rm = TRUE)) +
  theme_minimal() +
  labs(title = "PSZ: Mean Average Difference (°C) of Sensor Duplicates",
       x = "Tree 1 Sensors",
       y = "Tree 2 Sensors",
       fill = "Mean Abs Diff") +
...
```{r}
TSZ_ind_sensor_tree1_n <- TSZ_threeday_accu_comp %>%
  select(-c(solarNoon, sunrise, sunset, dawn, dusk)) %>%
  filter(tree_name == "Tree 1", aspect == "north") %>%
  pivot_wider(names_from = burn_status, 
              values_from = temp) %>%
...
```{r}
TSZ_ind_sensor_tree1_s <- TSZ_threeday_accu_comp %>%
  select(-c(solarNoon, sunrise, sunset, dawn, dusk)) %>%
  filter(tree_name == "Tree 1", aspect == "south") %>%
  pivot_wider(names_from = burn_status, 
              values_from = temp) %>%
...
```{r}
TSZ_ind_sensor_tree2_n <- TSZ_threeday_accu_comp %>%
  select(-c(solarNoon, sunrise, sunset, dawn, dusk)) %>%
  filter(tree_name == "Tree 2", aspect == "north") %>%
  pivot_wider(names_from = burn_status, 
              values_from = temp) %>%
...
```{r}
#south tree 2 
TSZ_ind_sensor_tree2_s <- TSZ_threeday_accu_comp %>%
  select(-c(solarNoon, sunrise, sunset, dawn, dusk)) %>%
  filter(tree_name == "Tree 2", aspect == "south") %>%
  pivot_wider(names_from = burn_status, 
              values_from = temp) %>%
...
ggplot(pairwise_diffs_ind, aes(x = var1, y = var2, fill = mean_diff)) +
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                       midpoint = mean(pairwise_diffs_ind$mean_diff, na.rm = TRUE)) +
  theme_minimal() +
  labs(title = "TSZ: Mean Average Difference (°C) of Sensor Duplicates",
       x = "Tree 1 Sensors",
       y = "Tree 2 Sensors",
       fill = "Mean Abs Diff") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
PSZ_ind_MAD <- PSZ_ind_comb %>%
  ...
  summarize(DB_MAD = abs(DB_Tree1 - DB_Tree2), 
            GB_MAD = abs(GB_Tree1 - GB_Tree2), 
            live_MAD = abs(live_Tree1 = live_Tree2, 
            GB_DB_MAD)
```
...
rename_vars <- c(
  ...
  "GB_north" = "GB North",
  "GB_south" = "GB South",
  "live_north" = "Live North",
  "live_south" = "Live South"
)
...
diffs_PSZ_day_heatmap <- ggplot(pairwise_diffs_PSZ_day, aes(x = var1, y = var2, fill = mean_diff)) +
  geom_tile() +  # Create heatmap tiles
  geom_text(aes(label = round(mean_diff, 2)), color = "black", size = 4) +  # Add values in squares
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                       midpoint = 3,
                       limits = color_limits) +  # Add color limits here
  theme_minimal() +
  labs(title = "a) PSZ Day",
       x = NULL,
       y = NULL,
       fill = "Mean Avg Diff (°C)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        strip.text = element_text(size = 11),  # Facet title size
        axis.title = element_text(size = 11),  # Axis title size
        axis.text = element_text(size = 11),  # Axis tick labels
...
diffs_TSZ_day_heatmap <- ggplot(pairwise_diffs_TSZ_day, aes(x = var1, y = var2, fill = mean_diff)) +
  geom_tile() +  # Create heatmap tiles
  geom_text(aes(label = round(mean_diff, 2)), color = "black", size = 4) +
   scale_fill_gradient2(low = "blue", mid = "white", high = "red", 
                       midpoint = 3, 
                       limits = color_limits) + 
  theme_minimal() +
...
PSZ_comb_heatmap <- ggarrange(diffs_PSZ_day_heatmap, diffs_PSZ_night_heatmap, nrow =1, ncol = 2, common.legend = TRUE, legend = "right", 
  ...
  heights = c(2, 2.5)    # Set equal height for both plots
)
...
```{r}
full_comb_heatmap <- ggarrange(PSZ_comb_heatmap, TSZ_comb_heatmap, nrow =2, ncol = 1, common.legend = TRUE, legend = "right")
