...
```{r}
full_pers_gbdbub_data <- read_csv("nsf/treetemp_data/full_pers_gbdbub_data.csv") %>%
  mutate(datetime = as.POSIXct(datetime, format=("%Y-%m-%d %H:%M:%S")),
                               datetime = with_tz(datetime, tz = 'MST'), ,
    day = as.Date(datetime, tz= "MST")) %>%
  inner_join(p_b_solar_times, by = "day") %>%
...
full_trans_gbdbub_data <- read_csv("nsf/treetemp_data/full_trans_gbdbub_data.csv") %>%
  mutate(datetime = as.POSIXct(datetime, format=("%Y-%m-%d %H:%M:%S")),
                               datetime = with_tz(datetime, tz = 'MST'), ,
    day = as.Date(datetime, tz= "MST")) %>%
  inner_join(p_b_solar_times, by = "day") %>%
...
PSZ_dtnt_treetemp_daily_gbdbub_lineplot <- ggplot(data = PSZ_dtnt_daily_gbdbub_plot_dataframe ) +
  ...
  scale_y_continuous(limits = c(-20, 25)) +  # Set the y-axis limits here
  theme_bw() + 
...
```{r}
#combining day and night plots
final_dtnt_treetemp_lineplot <- ggarrange(
  PSZ_dtnt_treetemp_daily_gbdbub_lineplot, 
  TSZ_dtnt_treetemp_daily_gbdbub_lineplot, 
  nrow = 2, 
  ncol = 1, 
...
print(final_dtnt_treetemp_lineplot)
```
...
```{r}
test <- t_accu_dr_gbdbub_LWout %>%
  group_by(day) %>%
  summarize(n = n(), value = mean(accu_daily_gb_LWout_MJperd, na.rm = TRUE)) %>%
  filter(n > 1)
```

```{r}
cum_t_exploration <- ggplot(data = t_accu_dr_gbdbub_LWout) +
  geom_line(aes(x = day, y = accu_daily_gb_LWout_MJperd)) + 
...
\*\* as of 2025-01-13 - need to look through this dataframe for TSZ and figure out why the LWout values are equal through march... this could be an issue with missing data?
