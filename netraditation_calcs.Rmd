...
#the RL_in units are W/m2, the m2 component is 
  ...
  mutate(time = format(datetime, "%H:%M:%S") %>% hms())
```
...
```{r}
#parsing out daytime/nightime dataframe
...
dtnt_avg_energy_plot <- ggplot(dtnt_max_energy_bysite, aes(x = date, y = daily_max_energy_MJ, color = burned_status, linetype = zone)) +
  geom_line() +   # Add lines
