...
\*\* need to work on these so I don't facet wrap them but rather ggarrange them!
...
filtered_pers_b_wx <- filtered_pers_b_wx %>%
  ...
  mutate(
    phase = case_when(
      time_hms >= hms("08:30:00") & time_hms < hms("17:30:00") ~ "day",
      time_hms < hms("08:30:00") | time_hms >= hms("17:30:00") ~ "night",
      TRUE ~ NA_character_  # Handle cases where none of the conditions match
    )
  ) %>%
  select(datetime, date, time, phase, everything())
...
#making one combined dataframe to pull from it needed
...
```{r}
#persistent burned individual combined wx/radiation dataframe
s_daily_avg_persb_tree <- s_daily_energy_burned_unburned %>%
  filter(zone == "persistent", burn_status == "burned")

filtered_persb_wx_rad <- filtered_pers_b_wx %>%
  full_join(s_daily_avg_persb_tree) %>%
  drop_na() %>%
  select(datetime, date, time, phase, everything()) %>%
  select(-time_hms)

#transitional unburned
#persistent burned individual combined wx/radiation dataframe
s_daily_avg_transub_tree <- s_daily_energy_burned_unburned %>%
  filter(zone == "transitional", burn_status == "unburned")

filtered_transub_wx_rad <- filtered_trans_ub_wx %>%
  full_join(s_daily_avg_transub_tree) %>%
  drop_na() %>%
  select(datetime, date, time, phase, everything()) %>%
  select(-time_hms)
```
...
```{r}
#testing the above dataframe to see if there are trans_ub issues
##trans_unburned
test_plot_daily_V4 <- ggplot(data = daily_avg_combined_R_data) +
  ...
  theme_minimal() +  
  facet_wrap(~ phase + zone + burn_status) +
...
-   there is something going on in the step where I create a daily combined dataset that is messing up the trans_ub data.

-   Try creating daily average dataframes individually for all zones then plot from those? - this did not work to fix the problem!

```{r}
#testing the transitional unburned data - shows that there is no specific flat section of data in the daily data
trans_ub_wx_plot_V2 <- ggplot(data = filtered_transub_wx_rad) +
  ...
  geom_line(aes(x = date, y = LWout_Avg, color = "LWout_Avg", linetype = burn_status)) + 
    geom_line(aes(x = date, y = tree_LWout_wperm2, color = "tree_LWout_wperm2", linetype = burn_status))
...
\*\* this step is important, use these plots for the ggarranged final plots
...
\
HAVEN'T WORKED ON ANYTHING ELSE BELOW THIS POINT AS OF 8/16/24
