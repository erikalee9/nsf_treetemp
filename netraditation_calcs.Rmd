...
filtered_pers_ub_aspects <- pers_ub_aspects %>%
  filter(datetime >= start_time & datetime <= end_time) %>%
  mutate(zone = "persistent", burn_status = "unburned")
...
filtered_trans_ub_aspects <- trans_ub_aspects %>%
  filter(datetime >= start_time & datetime <= end_time) %>%
  mutate(zone = "transitional", burn_status = "unburned")
```
...
```{r}
#creating a daily w/m2 dataframe for the trees LWout values - to be used later with the weather station radiation components and plotting!
daily_avg_tree_LWout_wperm2 <- all_treetemps %>%
  select(datetime, date, time, zone, burn_status, tree_name, aspect, tree_RL_out) %>%
  rename(tree_LWout_wperm2 = "tree_RL_out") %>%
  ...
  summarize(tree_LWout_avg_wperm2 = mean(tree_LWout_wperm2, na.rm = TRUE))
```

```{r}
# Aggregate energy (J) to get daily total energy in Joules
##this daily total energy is the LWout from the trees!
daily_energy_MJ <- all_treetemps %>%
  group_by(date, time, tree_name, zone, burn_status, aspect) %>%
...
average_daily_energy_by_site <- daily_energy_MJ %>%
  group_by(date, time, zone, burn_status) %>%
...
```{r}
#don't use this plot, use the one below that is facet-wrapping by zone and phase
#plotting daytime/nighttime plot
...
```{r}
#south-only dataframes, arranged correctly with facet wrap by zone and phase

#plotting daytime/nighttime plot
s_dtnt_avg_e_plot_V2 <- ggplot(s_dtnt_avg_e, aes(x = date, y = daily_avg_energy_MJ, color = burn_status)) +
  geom_line() +   # Add lines
  labs(title = "South Aspect Average LWout from trees by day/night",
       x = "Date",
       y = "Avg RL out (MJ)",
       color = "Burn Status",
       linetype = "Zone") +
...
s_dtnt_avg_e_plot_V2
```

```{r}
#south max LWout day/night plot
s_dtnt_max_e_plot_V2 <- ggplot(s_dtnt_max_e, aes(x = date, y = daily_max_energy_MJ, color = burn_status)) +
  geom_line() +   # Add lines
  labs(title = "South Aspect Maximum LWout from trees by day/night",
       x = "Date",
       y = "Max RL out (MJ)",
       color = "Burn Status") +
  theme_minimal() +
...
full_LWout_data <- all_treetemps %>%
  select(datetime, aspect, site, tree_RL_out, energy_J) %>%
  mutate(
