...
three_custom_colors <- c("#D95F02","#2166AC", adjustcolor("#1B7837", alpha = 0.8))

## orange is burned, blue is unburned
two_custom_colors <- c("#D95F02", adjustcolor("#1B7837", alpha = 0.8))
```
...
#the RL_in units are W/m2, the m2 component is 
  ...
  # Convert instantaneous power (W) to energy (J/day)
  mutate(treeLWout_Jperd = W * 86400) %>%
  mutate(treeLWout_MJperd = treeLWout_Jperd / 1e6) %>%
...
avg_s_all_treetemps <- all_treetemps %>%
  ...
  # Calculate the mean of all south aspect temps by burn_status
  summarize(
    temp = mean(temp, na.rm = TRUE), 
    temp_k = mean(temp_k, na.rm = TRUE), 
    tree_LWout_wperm2 = mean(tree_RL_out, na.rm = TRUE), 
    W = mean(W, na.rm = TRUE), 
    treeLWout_Jperd = mean(treeLWout_Jperd, na.rm = TRUE), treeLWout_MJperd = mean(treeLWout_MJperd, na.rm = TRUE)
  ) %>%
  ungroup() %>%
...
s_daily_avg_tree_e <- avg_s_all_treetemps %>%
  select(datetime, date, time, phase, zone, burn_status, temp, tree_LWout_wperm2, treeLWout_Jperd, treeLWout_MJperd) %>%
  #grouping to get daily average
  group_by(date, phase, burn_status, zone) %>%
  summarize(temp_C = mean(temp, na.rm = TRUE), tree_LWout_wperm2 = mean(tree_LWout_wperm2, na.rm = TRUE),
            tree_LWout_Jperd = mean(treeLWout_Jperd, na.rm = TRUE), tree_LWout_MJperd = mean(treeLWout_MJperd, na.rm = TRUE))

#daytime only dataframe - to use in V2 plots below
s_dt_daily_avg_tree_e <- s_daily_avg_tree_e %>%
  filter(phase == "day")
```

```{r}
#exporting above dataframe for use in radiation plot comparisons - this dataframe has gb/db/ub values. 
write.csv(s_daily_avg_tree_e, "nsf/105E_trans_burned/day_avg_filtered_treeLWout_data_gbdbub.csv")
```

# Final Day/Night Plots

-   \*\* Important plots, using these in thesis!
-   V2 versions of below plot are only doing the daytime phase

```{r}
#MJ plot
s_dtnt_avg_e_MJ_plot <- ggplot(s_daily_avg_tree_e, aes(x = date, y = tree_LWout_MJperd, color = burn_status), linewidth = 0.8) +
  geom_line() +   # Add lines
...
```{r}
#w per m2 plot
s_dtnt_avg_e_wperm2_plot <- ggplot(s_daily_avg_tree_e, aes(x = date, y = tree_LWout_wperm2, color = burn_status), linewidth = 0.8) +
  geom_line() +   # Add lines
...
```{r}
s_dt_avg_e_wperm2_V2 <- ggplot(s_dt_daily_avg_tree_e) +
  # Map both color and linetype by burn_status
  geom_line(aes(x = date, y = tree_LWout_wperm2, color = burn_status, linetype = burn_status), linewidth = 0.8) +  
  
  labs(#title = "South Aspect 24-hour Avg LWout (W/m2) from Trees",
       x = "Date",
       y = "Daily Avg Tree LWout (W/m2)",
       color = "Burn Status") +
  theme_bw() +
  facet_wrap(~ zone + phase, dir = "h") +
  
  # Customizing color and linetype by burn_status
  scale_color_manual(values = c("dead burn" = "#D95F02", "green burn" = "#2166AC", "live" = "#1B7837")) +
  scale_linetype_manual(values = c("dead burn" = "solid", "green burn" = "solid", "live" = "dashed")) +

  # Remove the title of the color legend
  guides(color = guide_legend(title = NULL), 
         linetype = guide_legend(title = NULL))  # Remove the title for linetype as well

s_dt_avg_e_wperm2_V2
```

```{r}
s_dt_avg_e_MJd_V2 <- ggplot(s_dt_daily_avg_tree_e) +
  # Map both color and linetype by burn_status
  geom_line(aes(x = date, y = tree_LWout_MJperd, color = burn_status, linetype = burn_status), linewidth = 0.8) +  
  
  labs(#title = "South Aspect 24-hour Avg LWout (MJ/d) from Trees",
       x = "Date",
       y = "Daily Avg Tree LWout (MJ/d)",
       color = "Burn Status") +
  theme_bw() +
  facet_wrap(~ zone + phase, dir = "h") +
...
s_daily_energy <- avg_s_all_treetemps %>%
  rename(tree_LWout_MJperd = treeLWout_MJperd) %>%
  select(datetime, phase, zone, burn_status, temp, tree_LWout_wperm2, tree_LWout_MJperd) %>%
  mutate(
...
s_daily_energy_burned <- s_daily_energy %>%
  ...
  summarise(
    ...
    tree_LWout_MJperd = mean(tree_LWout_MJperd, na.rm = TRUE)
  ) %>%
...
```{r}
#exporting tree_LWout dataframe to save as csv in r
write_csv(day_avg_filtered_tree_LWout_data, "nsf/105E_trans_burned/day_avg_filtered_treeLWout_data_bub.csv")
```
...
burned_day_s_avg_plot <- ggplot(data = burned_day_s_avg_values) + 
  geom_line(aes(x = date, y = SWin_Avg, color = "SWin_Avg", linetype = "Weather Station"), size = 0.6, alpha = 0.9) +
  geom_line(aes(x = date, y = SWout_Avg, color = "SWout_Avg", linetype = "Weather Station"), size = 0.6, alpha = 0.8) +
  geom_line(aes(x = date, y = LWin_Avg, color = "LWin_Avg", linetype = "Weather Station"), size = 0.7, alpha = 0.9) +
  geom_line(aes(x = date, y = LWout_Avg, color = "LWout_Avg", linetype = "Weather Station"), size = 0.7) +
...
# Day Night Tree Temp Aspect Plots

```{r}
#creating dataframes for only north and south aspects, with averaged day and night temps

filtered_all_treetemps <- all_treetemps %>%
   mutate(
    # Extract the date part
    date = as.Date(datetime),
    # Extract the time part
...
#daytime temps
filtered_day_treetemps <- filtered_all_treetemps %>%
  #filtering out charred/uncharred temps
  filter(aspect %in% c("north", "south")) %>%
  select(-sensor_id) %>%
  group_by(date, zone, burn_status, phase, aspect) %>%
  summarize(across(everything(), mean, na.rm = TRUE)) %>%
  select(date, phase, zone, burn_status, aspect, temp) %>%
  filter(phase == "day") %>%
  rename(avg_treetemp = temp)

#nighttime temps
filtered_night_treetemps <- filtered_all_treetemps %>%
  #filtering out charred/uncharred temps
  filter(aspect %in% c("north", "south")) %>%
  select(-sensor_id) %>%
  group_by(date, zone, burn_status, phase, aspect) %>%
...
```{r}
#creating a linetype
aspect <-c("north" = "dashed", "south" = "solid")
```

Option 1 for plotting

```{r}
#plotting aspect averages by day/night and north/south
day_treetemp_plots <- ggplot(data = filtered_day_treetemps) + 
  geom_line(aes(x = date, y = avg_treetemp, color = burn_status, linetype = aspect)) +
  labs(
    title = "Daytime Avg Tree Temps",
   x = "Date",
   y = "Temperature (C)",
   color = "Burn Status", 
   linetype = "Aspect"
  ...
  theme_bw() +
  facet_wrap(~ zone, dir = "h") +
  scale_color_manual(values = three_custom_colors) +
  scale_linetype_manual(values = aspect)

day_treetemp_plots
```

```{r}
night_treetemp_plots <- ggplot(data = filtered_night_treetemps) + 
  geom_line(aes(x = date, y = avg_treetemp, color = burn_status, linetype = aspect)) +
  labs(
    title = "Nighttime Avg Tree Temps",
   x = "Date",
   y = "Temperature (C)",
   color = "Burn Status", 
   linetype = "Aspect"
...
night_treetemp_plots
```

```{r}
#alligning the day and night plots

comb_aspect_treetemps <- ggarrange(day_treetemp_plots, night_treetemp_plots, ncol = 1, nrow = 2, common.legend = TRUE, legend = "right")

comb_aspect_treetemps
```

```{r}
#saving combined plot
#saving ggplot as high quality png
ggsave("comb_aspect_treetemps.png", comb_aspect_treetemps, width = 8, height = 6, dpi = 300)
```

Option 2 for plotting - more simple day/night comparison so it does not repeat the gb/db/ub plots?

```{r}
#manipulating dataframe first
##filtered burned day
filtered_burned_treetemps <- filtered_all_treetemps %>%
  #filtering out charred/uncharred temps
  filter(aspect %in% c("north", "south")) %>%
  select(-sensor_id) %>%
  #creating a dataframe with only burned temps, then getting their average
  filter(burn_status %in% c("green burn", "dead burn")) %>%
  group_by(date, zone, phase, aspect) %>%
  summarize(avg_treetemp = mean(temp, na.rm = TRUE), .groups = 'drop') %>%
  mutate(burn_status = "burned") %>%
  select(date, phase, zone, burn_status, aspect, avg_treetemp)

filtered_burned_day_treetemps <- filtered_burned_treetemps %>%
  filter(phase == "day")

filter_burned_night_treetemps <- filtered_burned_treetemps %>%
  filter(phase == "night")


##filtered unburned 
filtered_unburned_treetemps <- filtered_all_treetemps %>%
  #filtering out charred/uncharred temps
  filter(aspect %in% c("north", "south")) %>%
  select(-sensor_id) %>%
  #creating a dataframe with only burned temps, then getting their average
...
filtered_unburned_day_treetemps <- filtered_unburned_treetemps %>%
  filter(phase == "day")

filtered_unburned_night_treetemps <- filtered_unburned_treetemps %>%
  filter(phase == "night")
```

```{r}
#combining plots so I can plot as day/nigth and do a simple facet wrap

combined_day_b_ub_treetemp_data <- filtered_burned_day_treetemps %>%
  full_join(filtered_unburned_day_treetemps)


combined_night_b_ub_treetemp_data <-
  filtered_burned_night_treetemps %>%
  full_join(filtered_unburned_night_treetemps)
```

```{r}
#creating aspect dependent dataframes to make more clear legends on V3 plots below
n_filtered_burned_day_tt <- filtered_burned_treetemps %>%
  filter(aspect == "north")

s_filtered_burned_day_tt <- filtered_burned_treetemps %>%
  filter(aspect == "south")

n_filtered_unburned_day_tt <- filtered_unburned_treetemps %>%
  filter(aspect == "north")

s_filtered_unburned_day_tt <- filtered_unburned_treetemps %>%
  filter(aspect == "south")
```
...
```{r}
#USE THIS PLOT ONLY! V3
{r}
daynight_treetemp_plots_V3 <- ggplot() + 
  geom_line(data = n_filtered_burned_day_tt, 
            aes(x = date, y = avg_treetemp, color = "North, burned", linetype = "dashed")) +
  geom_line(data = s_filtered_burned_day_tt, 
            aes(x = date, y = avg_treetemp, color = "South, burned", linetype = "solid"), alpha = 0.8, linewidth = 0.7) + 
  geom_line(data = n_filtered_unburned_day_tt, 
            aes(x = date, y = avg_treetemp, color = "North, live", linetype = "dashed")) +
  geom_line(data = s_filtered_unburned_day_tt, 
            aes(x = date, y = avg_treetemp, color = "South, live", linetype = "solid"), alpha = 0.8, linewidth = 0.7) +
  labs(
    x = "Date",
    y = "Temperature (C)"
  ) +
  ...
  # Customizing the color and linetype legend
  scale_color_manual(name = NULL,  # Remove header for color legend
                     values = c("North, burned" = "#D95F02", "South, burned" = "#D95F02", 
                                "North, live" = "#1B7837", "South, live" = "#1B7837")) +
  scale_linetype_manual(values = c("dashed" = "dashed", "solid" = "solid"), 
                        guide = "none") +  # Remove linetype legend
  guides(color = guide_legend(override.aes = list(linetype = c("dashed", "dashed", "solid", "solid")), 
                              title = NULL))  # Remove color legend header

daynight_treetemp_plots_V3
```

```{r}
#daytime plot
day_treetemp_plots_V2 <- ggplot(data = combined_day_b_ub_treetemp_data) + 
  geom_line(aes(x = date, y = avg_treetemp, color = burn_status, linetype = aspect), alpha = 0.8) +
  labs(
    title = "Daytime Avg Tree Temps",
   x = "Date",
   y = "Temperature (C)",
...
day_treetemp_plots_V2
```

```{r}
#nighttime plot
night_treetemp_plots_V2 <- ggplot(data = combined_night_b_ub_treetemp_data) + 
  geom_line(aes(x = date, y = avg_treetemp, color = burn_status, linetype = aspect), alpha = 0.8) +
  labs(
    title = "Nighttime Avg Tree Temps",
   x = "Date",
...
night_treetemp_plots_V2
```

```{r}
#alligning the day and night plots

comb_aspect_treetemps_V2 <- ggarrange(day_treetemp_plots_V2, night_treetemp_plots_V2, ncol = 1, nrow = 2, common.legend = TRUE, legend = "right")

comb_aspect_treetemps_V2
```

```{r}
#saving combined plot
#saving ggplot as high quality png
ggsave("comb_aspect_treetemps_V2.png", comb_aspect_treetemps_V2, width = 8, height = 6, dpi = 300)
```
