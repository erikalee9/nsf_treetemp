...
s_daily_avg_tree_e <- avg_s_all_treetemps %>%
  select(datetime, date, time, phase, zone, burn_status, tree_LWout_wperm2, treeLWout_Jperd, treeLWout_MJperd) %>%
  #grouping to get daily average
  group_by(date, phase, burn_status, zone) %>%
  summarize(tree_LWout_wperm2 = mean(tree_LWout_wperm2, na.rm = TRUE),
