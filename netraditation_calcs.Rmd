...
start_time <- ymd_hms("2024-02-02 00:15:00")
...
\*\* the above data for trans_b is still not working correctly! Why is this?

\*\* Individually adding in date, time and phase column to try to fix day/night SW issue.

```{r}
filtered_trans_b_wx_V2 <- filtered_trans_b_wx %>%
  mutate(
    # Extract the date part
    date = date(datetime),
    # Extract the time part
