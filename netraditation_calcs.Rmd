...
end_time <- ymd_hms("2024-05-24 00:00:00", tz = "America/Denver")
...
avg_s_all_treetemps <- all_treetemps %>%
  ...
  mutate(
    ...
    phase = case_when(
      time_hms >= hms::as_hms("08:30:00") & time_hms < hms::as_hms("17:30:00") ~ "day",
      time_hms < hms::as_hms("08:30:00") | time_hms >= hms::as_hms("17:30:00") ~ "night",
      TRUE ~ NA_character_
    ),
...
-   Daytime from 8:30:00 to 17:30:00 MST

```{r}
#creating a day-only combined dataframe to use for radiation and weather station plots
day_combined_filtered_wx_data <- combined_filtered_wx_data %>%
  filter(phase == "day")
```
...
##Use this for the radiation and LWout plots!
day_final_combined_wx_treeLWout_data <- day_avg_filtered_wx_data %>%
...
arranged_plots <- ggarrange(burned_day_s_avg_plot, unburned_day_s_avg_plot, ncol = 1, nrow = 2, common.legend = TRUE, legend = "right")
