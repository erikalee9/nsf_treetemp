...
t_mean_NetLW_wotree_plot <- ggplot() + 
  # Column plot for Net Longwave Radiation of total plot area
  geom_col(data = mean_dt_transb_radtree_comb, aes(x = date, y = total_mean_netLW_wotree_MJd), fill = "#1B7837", position = "stack", alpha = 0.7) +
  # Line plot for Total burned trees LWout
  geom_line(data = mean_dt_transb_radtree_comb, aes(x = date, y = total_mean_tree_LWout_MJd, color = "Total burned trees LWout (MJ/d)")) +
  # Line plot for snow cam max depth on a secondary y-axis
  geom_line(data = filtered_transb_snowcamera_data, aes(x = date, y = snowcam_max_depth_m * depth_factor, 
...
mean_NR_acc_plot <- ggplot(data = accum_mean_data_bothzones) +
  ...
  geom_line(aes(x = date, y = accu_netRad_wotree_MJd, color = "Net Radiation", linetype = "Net Radiation"), linewidth = 0.6) +
  facet_wrap(~zone + burn_status) +
  labs(
    title = "Accumulation Mean Data Values",
    x = "Date",
    y = "Radiation Component (MJ)",
    color = "Radiation Data",  # Legend title for color
    linetype = "Radiation Data"  # Legend title for linetype
  ) +
  scale_color_manual(
    values = c("Tree LWout" = "#D95F02", 
               "Net LW" = "#1B7837", 
               "Net SW" = "#2166AC", 
               "Net Radiation" = "#666666")
  ) +
  scale_linetype_manual(
    values = c("Tree LWout" = "solid", 
               "Net LW" = "solid", 
               "Net SW" = "solid", 
               "Net Radiation" = "dashed")) +
