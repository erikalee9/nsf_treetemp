...
accum_mean_data_bothzones <- rbind(mean_dt_transb_radtree_comb, mean_dt_persb_radtree_comb) %>%
  select(date, phase, zone, burn_status, accu_netSW_MJd, accu_netLW_wtree_MJd, accu_netLW_wotree_MJd, accu_netRad_wtree_MJd, accu_netRad_wotree_MJd, accu_treeLWout_MJd)
```
...
accu_transb_treewell_dates <- persb_snowcamera_data %>%
  filter(date >= as.Date("2024-02-02") & date <= as.Date("2024-05-24")) %>%
  select(date, zone, burn_status, snowcam_max_depth_cm, conditions, treewell_developement)
...
mean_NR_acc_plot <- ggplot(data = accum_mean_data_bothzones) +
  geom_line(aes(x = date, y = accu_treeLWout_MJd, color = "Tree LWout", linetype = "Tree LWout"), linewidth = 0.7) +
  geom_line(aes(x = date, y = accu_netLW_wotree_MJd, color = "Net LW", linetype = "Net LW"), linewidth = 0.7) +
  geom_line(aes(x = date, y = accu_netSW_MJd, color = "Net SW", linetype = "Net SW"), linewidth = 0.7) + 
  geom_line(aes(x = date, y = accu_netRad_wotree_MJd, color = "Total Net Radiation", linetype = "Total Net Radiation"), linewidth = 0.7) +
  geom_point(data = accu_treewell_data, aes(x = date, y = accu_treeLWout_MJd, 
             shape = "Dates that treewells developed"), 
             color = "black", fill = "black", size = 1.5) +
  facet_wrap(~zone + burn_status) +
  labs(
    title = "Accumulation Mean Data Values w/o Tree LWout in Net Rad",
    x = "Date",
...
mean_NR_acc_plot <- ggplot(data = accum_mean_data_bothzones) +
  ...
  geom_line(aes(x = date, y = accu_netLW_wotree_MJd, color = "Net LW", linetype = "Net LW"), linewidth = 0.7) +
  geom_line(aes(x = date, y = accu_netSW_MJd, color = "Net SW", linetype = "Net SW"), linewidth = 0.7) + 
  geom_line(aes(x = date, y = accu_netRad_wtree_MJd, color = "Total Net Radiation", linetype = "Total Net Radiation"), linewidth = 0.7) +
  geom_point(data = accu_treewell_data, aes(x = date, y = accu_treeLWout_MJd, 
             shape = "Dates that treewells developed"), 
             color = "black", fill = "black", size = 1.5) +
  facet_wrap(~zone + burn_status) +
  labs(
    title = "Accumulation Mean Data Values with Tree LWout in Net Rad",
    x = "Date",
    y = "Radiation Component (MJ/d)"
  ) +
  scale_color_manual(
