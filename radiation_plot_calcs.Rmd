...
trans_mean_NR_accum_data <- mean_dt_transb_radtree_comb %>%
  arrange(date) %>%
  mutate(accu_netSW_MJd = accumulate(mean_wx_netSW_MJd, "+"))
```

```{r}
#transitional data wrangling
pers_mean_NR_accum_data <- mean_dt_persb_radtree_comb %>%
  arrange(date) %>%
  mutate(accu_netSW_MJd = accumulate(mean_wx_netSW_MJd, "+"))
```
