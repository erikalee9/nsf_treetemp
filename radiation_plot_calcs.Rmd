...
transb_treewell_dates <- filtered_transb_snowcamera_data %>%
  ...
  inner_join(final_dt_transb_radtree_comb_data) %>%
...
##next step here would be to multiply by LWout of tree (W/m2) value to get Watz impacted by tree... Before I do this I need to look at how I converted the LWout MJ/d for the tree data first and make sure I am not repeating steps? Look at workflow... may just be able to take temp and convert using equation given by stephanie and used in old calcs ONCE the values are converted to get rid of area?
```
...
mean_dt_transb_radtree_comb <- avg_dt_transb_rad_wx %>%
  ...
  mutate(total_mean_netLW_MJd = mean_wx_netLW_MJd + total_mean_tree_LWout_MJd, total_mean_net_rad = mean_wx_NRin_MJd - mean_wx_NRout_MJd - total_mean_tree_LWout_MJd)
  
mean_dt_transb_radtree_comb_data <- dt_transb_radtree_comb %>%
  select(-c(6:12)) %>%
...
Everything in the next chunk of code has not been run yet! Make sure I am doing this correctly, then I can run it once I figure out the mean_data
...
mean_dt_transb_radtree_comb <- avg_dt_transb_rad_wx %>%
  ...
  #creating a net radiation weather station parameter
  mutate(NR_MJd = SWin_MJd - SWout_MJd + LWin_MJd - LWout_MJd) %>%
  select(date, phase, zone, burn_status, SnowDepth_m, SWin_MJd, SWout_MJd, SWnet_MJd, SWalbedo_Avg, LWin_MJd, LWout_MJd, LWnet_MJd, NR_MJd, total_min_tree_LWout_MJd, total_max_tree_LWout_MJd, total_mean_tree_LWout_MJd, total_min_area_m2, total_max_area_m2, total_mean_area_m2) %>%
  rename(total_min_tree_area_m2 = total_min_area_m2, total_max_tree_area_m2 = total_max_area_m2, total_mean_tree_area_m2 = total_mean_area_m2) %>%
  #calculating the area of the weather station region's impacts 
  mutate(total_min_wx_area_m2 = 1125-total_min_tree_area_m2, total_max_wx_area_m2 = 1125-total_max_tree_area_m2, total_mean_wx_area_m2 = 1125-total_mean_tree_area_m2) %>%
  #calculating the net outgoing rad for the weather station data and removing the m2 unit by multiplying by the area that this weather station data is applied to
  mutate(min_wx_NRl_MJd =  (SWout_MJd + LWout_MJd)* total_min_wx_area_m2, max_wx_NRl_MJd = (SWout_MJd + LWout_MJd)* total_max_wx_area_m2, mean_wx_NRl_MJd = (SWout_MJd + LWout_MJd) * total_mean_wx_area_m2) %>%
  #calcuating the total net longwave radiation for the plot area - this will be the y-axis values
  mutate(total_min_NRl_MJd = min_wx_NRl_MJd + total_min_tree_LWout_MJd, total_max_NRl_MJd = max_wx_NRl_MJd + total_max_tree_LWout_MJd, total_mean_NRl_MJd = mean_wx_NRl_MJd + total_mean_tree_LWout_MJd)
...
mean_NR_plot <- ggplot() + 
  geom_line(data = final_dt_transb_radtree_comb_data, aes(x = date, y = total_mean_NRl_MJd, color = "Net Longwave Radiation of total plot area")) +  
  geom_line(data = final_dt_transb_radtree_comb_data, aes(x = date, y = total_mean_tree_LWout_MJd, color = "Total burned trees LWout")) +
...
mean_NetLW_plot <- ggplot() + 
  ...
  geom_col(data = mean_dt_transb_radtree_comb, aes(x = date, y = total_mean_netLW_MJd, fill = "Net Longwave Radiation of total plot area"), position = "stack", alpha = 0.8) +
...
mean_NetSW_plot <- ggplot() + 
  # Column plot for Net Longwave Radiation of total plot area
  geom_col(data = mean_dt_transb_radtree_comb, aes(x = date, y = mean_wx_netSW_MJd, fill = "Net Shortwave Radiation of total plot area"), position = "stack", alpha = 0.8) +
  # Line plot for Total burned trees LWout
  geom_line(data = mean_dt_transb_radtree_comb, aes(x = date, y = total_mean_tree_LWout_MJd, color = "Total burned trees LWout")) +
  # Point plot for treewell development dates
  geom_point(data = transb_treewell_dates, aes(x = date, y = total_mean_tree_LWout_MJd, shape = "Dates that treewells developed"), fill = "black", size = 1) + 
...
mean_NetSW_plot <- ggplot() + 
  # Column plot for Net Longwave Radiation of total plot area
  geom_col(data = mean_dt_transb_radtree_comb, aes(x = date, y = total_mean_net_rad, fill = "Total Net Radiation of plot area"), position = "stack", alpha = 0.8) +
  # Line plot for Total burned trees LWout
  geom_line(data = mean_dt_transb_radtree_comb, aes(x = date, y = total_mean_tree_LWout_MJd, color = "Total burned trees LWout")) +
  # Point plot for treewell development dates
  geom_point(data = transb_treewell_dates, aes(x = date, y = total_mean_tree_LWout_MJd, shape = "Dates that treewells developed"), fill = "black", size = 1) + 
  ...
  scale_color_manual(name = "Radiation Data", 
                     ...
                     labels = c("Total burned trees LWout")) +
  scale_fill_manual(name = "Radiation Data", 
                     values = c("Total Net Radiation of plot area" = "#2166AC"),
