...
```{r}
#redoing calculations for radiation conversions from W/m2 to MJ/day
avg_dt_transb_rad_wx <- filtered_transb_rad_wx %>%
  group_by(date, phase) %>%
  summarize(across(everything(), mean, na.rm = TRUE)) %>%
  select(-c(datetime, time)) %>%
  group_by(date, phase) %>%
  # Convert mean radiation to MJ/m²/day
  mutate(SWin_MJd = SWin_Avg * 86400 / 1e6, SWout_MJd = SWout_Avg * 86400 / 1e6, LWin_MJd = LWin_Avg * 86400 / 1e6, LWout_MJd = LWout_Avg * 86400 / 1e6, SWnet_MJd = SWnet_Avg * 86400 / 1e6, LWnet_MJd = LWnet_Avg * 86400 / 1e6, NR_MJd = NR_Avg * 86400 / 1e6) %>%
...
```{r}
#I think these conversions are wrong! Use above dataframe, don't run this
avg_dt_transb_rad_wx <- filtered_transb_rad_wx %>%
  group_by(date, phase) %>%
  summarize(across(everything(), mean, na.rm = TRUE)) %>%
  select(-c(datetime, time)) %>%
