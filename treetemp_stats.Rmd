...
monthly_gb_data <- final_15min_gb_temps %>%
  group_by(month, zone, burn_status) %>%
  summarize(mean_north_temp = mean(north_temp, na.rm = TRUE), mean_south_temp = mean(south_temp, na.rm = TRUE), mean_charred_temp = mean(charred_temp, na.rm = TRUE), mean_uncharred_temp = mean(uncharred_temp, na.rm = TRUE), ns_avg_temp = mean((north_temp+south_temp/2), na.rm = TRUE), c_uc_avg_temp = mean((charred_temp + uncharred_temp/2), na.rm = TRUE), s_c_avg_temp = mean((south_temp+charred_temp/2), na.rm = TRUE), s_uc_avg_temp = mean((south_temp+uncharred_temp/2), na.rm = TRUE), n_c_avg_temp = mean((north_temp+charred_temp/2), na.rm = TRUE), n_uc_avg_temp = mean(north_temp+uncharred_temp/2), ns_temp_diff = mean(north_temp - south_temp), c_uc_temp_diff = mean(charred_temp - uncharred_temp), s_c_temp_diff = mean(south_temp - charred_temp), s_uc_temp_diff = mean(south_temp - uncharred_temp), n_c_temp_diff = mean(north_temp - charred_temp), n_uc_temp_diff = mean(north_temp - uncharred_temp), .groups = 'drop')
```

\*\* keep working on the below code, it is not working right now??
