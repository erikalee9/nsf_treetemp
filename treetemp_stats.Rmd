...
monthly_ns_data <- full_15min_ns %>%
  group_by(month, zone, burn_status) %>%
  summarize(mean_north_temp = mean(north_temp, na.rm = TRUE), mean_south_temp = mean(south_temp, na.rm = TRUE), ns_avg_temp = mean(ns_avg_temp, na.rm = TRUE), ns_temp_diff = mean(ns_temp_diff, na.rm = TRUE), .groups = 'drop')
```
...
stats_monthly_ns <- full_15min_ns %>%
  #calculating monthly stats
  group_by(month, zone, burn_status) %>%
  summarize(mean_north_temp = mean(north_temp), mean_south_temp = mean(south_temp), ns_mean_temp = mean(ns_avg_temp), mean_diff = mean(ns_temp_diff), sd = sd(ns_temp_diff, na.rm = TRUE),
...
```{r}
final_stats_monthly_ns_gb_table <- stats_monthly_ns_gb %>%
  rename("burn status" = burn_status, "north temp" = mean_north_temp, "south temp" = mean_south_temp, "mean temp" = ns_mean_temp, "mean temp difference" = mean_diff, "coefficient of variance" = coeff_var) %>%
  kbl() %>%
