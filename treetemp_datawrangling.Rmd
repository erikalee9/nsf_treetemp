...
\*\* the above process worked, now I just need to repeat it for the transitional zone.
...
final_persb_ns_temps <- final_persb_ns_temps %>%
  ...
  #percent difference - absolute difference (no negatives)
   mutate(percent_diff = abs(100*(temp_diff/mean(south_temp, north_temp))))

stats_persb_ns_temp <- final_persb_ns_temps %>%
  filter(!is.na(temp_diff) & temp_diff != 0) %>%  # Filter out NA and zero values
  summarise(
    avg_diff = mean(temp_diff, na.rm = TRUE),  # Calculate average difference
    sd_diff = sd(temp_diff, na.rm = TRUE),     # Calculate standard deviation for diagnostics
    cv_diff = ifelse(avg_diff != 0, sd_diff / avg_diff * 100, NA)  # Calculate CV, handling NA
  )
```
...
#calculating statistics
##temp difference
final_tranb_ns_temps <- final_transb_ns_temps %>%
  mutate(temp_diff = south_temp - north_temp) %>%
  #percent difference - absolute difference (no negatives)
   mutate(percent_diff = abs(100*(temp_diff/mean(south_temp, north_temp))))

stats_transb_ns_temp <- final_transb_ns_temps %>%
  filter(!is.na(temp_diff) & temp_diff != 0) %>%  # Filter out NA and zero values
  summarise(
    avg_diff = mean(temp_diff, na.rm = TRUE),  # Calculate average difference
    sd_diff = sd(temp_diff, na.rm = TRUE),     # Calculate standard deviation for diagnostics
