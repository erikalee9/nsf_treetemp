...
final_15min_gbdb <- final_persb_gbdb_temps %>% 
...
```{r}
# GB/DB/UB comparison
full_15min_gbdbub <- final_persb_gbdb_temps %>%
  full_join(final_transb_gbdb_temps)
...
```{r}
full_hourly_ns_b <- final_persb_ns_temps %>%
  ...
  group_by(datetime_hour, day, month, burn_status) %>%
...
full_hourly_ns_ub <- final_pers_ub_ns_temps %>%
full_join(final_trans_ub_ns_temps) %>%
  mutate(datetime = as.POSIXct(datetime, format = "%Y-%m-%d %H:%M:%S")) %>% 
  mutate(datetime_hour = floor_date(datetime, unit = "hour")) %>%
  mutate(day = format(datetime, "%Y-%m-%d"),
