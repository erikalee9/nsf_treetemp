...
final_hourly_gb_temps <-final_gb_temps %>%
  ...
  summarize(north_temp = mean(north_temp), south_temp = mean(south_temp), charred_temp = mean(charred_temp), uncharred_temp = mean(uncharred_temp), ns_avg_temp = ((north_temp + south_temp)/2), sc_avg_temp = ((south_temp + charred_temp)/2), suc_avg_temp = ((south_temp + uncharred_temp)/2), nc_avg_temp = ((north_temp + charred_temp)/2), nuc_avg_temp = ((north_temp + uncharred_temp)/2), ns_temp_diff = (south_temp - north_temp), sc_temp_diff = (south_temp - charred_temp), suc_temp_diff = (south_temp - uncharred_temp), nc_temp_diff = (north_temp - charred_temp), nuc_temp_diff = (north_temp - uncharred_temp), .groups = 'drop')
```
 ...
 filter(datetime_hour >= as.POSIXct("2024-02-02 00:00:00", tz = "MST") & 
  ...
  mutate(
    phase = case_when(
      ...
      datetime_hour >= sunset ~ "night",
      TRUE ~ NA_character_)) %>%
  select(datetime_hour, day, month, phase, zone, burn_status, north_temp, south_temp, charred_temp, uncharred_temp, ns_avg_temp, sc_avg_temp, suc_avg_temp, nc_avg_temp, nuc_avg_temp, ns_temp_diff, sc_temp_diff, suc_temp_diff, nc_temp_diff, nuc_temp_diff, sunrise, sunset, solarNoon)
```
