...
filtered_pers_b_aspects <- pers_b_aspects %>%
  ...
  filter(datetime >= as.POSIXct("2024-02-02 00:00:00", tz = "MST") & 
         datetime <= as.POSIXct("2024-05-24 00:00:00", tz = "MST"))
...
filtered_pers_b_aspects <- filtered_pers_b_aspects %>%
  ...
  select(datetime, day, month, zone, burn_status, sensor_id, tree_name, everything()) %>%
  mutate(datetime = as.POSIXct(datetime, format = "%Y-%m-%d %H:%M:%S", tz = "MST"))
...
full_burned_combined_data <- filtered_pers_b_aspects %>%
  ...
  #issue with time parsing of trans_b so we need to start on 00:15:00
...
full_pers_gbdb_data <- full_burned_combined_data %>%
  ...
  summarize(mean_temp = mean(temp, na.rm = TRUE), .groups = 'drop') %>%
  mutate(datetime = as.POSIXct(datetime, format = "%Y-%m-%d %H:%M:%S", tz = "MST")) %>%
  filter(datetime <= as.POSIXct("2024-05-23 23:45:00", tz = "MST"))

final_pers_gbdb_temps <- full_pers_gbdb_data %>%
  group_by(datetime, day, month, aspect, zone) %>%
...
final_15min_gbdb <- final_pers_gbdb_temps %>%
  ...
  #mutating the floor_date to fix datetime issue
...
```{r}
dtnt_full_15min_ns <- full_15min_ns %>%
  pivot_wider(
    names_from = phase,
...
dt_alltemps <- dtnt_full_15min_ns %>%
  ...
  select(datetime, day, month, zone, burn_status, north_temp_day, south_temp_day) %>%
...
solex_dates <- seq(as.Date("2024-02-02"), as.Date("2024-05-24"), by = "day")
...
filtered_pers_b_aspects_r <- filtered_pers_b_aspects %>%
  ...
  mutate(
    phase = case_when(
      ...
      TRUE ~ NA_character_)) %>%
  select(datetime, day, month, phase, zone, burn_status, aspect, sensor_id, tree_name, temp, solarNoon, sunrise, sunset, dawn, dusk)
```
...
full_hourly_ns_dtnt <- full_hourly_ns %>%
  ...
  mutate(
   ...
   #filtering for the correct timeframe for both zones
 filter(datetime_hour >= as.POSIXct("2024-02-02 00:00:00", tz = "MST") & 
         datetime_hour <= as.POSIXct("2024-05-23 23:00:00", tz = "MST")) %>%
 ...
 filter(datetime >= as.POSIXct("2024-02-02 00:15:00", tz = "MST") & 
  ...
  mutate(
    phase = case_when(
      ...
      datetime >= sunset ~ "night",
      TRUE ~ NA_character_)) %>%
  select(datetime, day, month, phase, zone, aspect, gb_temp, db_temp, ub_temp, gbdb_avg_temp, gbub_avg_temp, dbub_avg_temp, gbdb_temp_diff, gbub_temp_diff, dbub_temp_diff, sunrise, sunset, solarNoon)
```
 ...
 filter(datetime_hour >= as.POSIXct("2024-02-02 00:00:00", tz = "MST") & 
  ...
  mutate(
    phase = case_when(
      ...
      datetime_hour >= sunset ~ "night",
      TRUE ~ NA_character_)) %>%
  select(datetime_hour, day, month, phase, zone, aspect, gb_temp, db_temp, ub_temp, gbdb_avg_temp, gbub_avg_temp, dbub_avg_temp, gbdb_temp_diff, gbub_temp_diff, dbub_temp_diff, sunrise, sunset, solarNoon)
```
 ...
 filter(datetime_hour >= as.POSIXct("2024-02-02 00:00:00", tz = "MST") & 
  ...
  mutate(
    phase = case_when(
      ...
      datetime_hour >= sunset ~ "night",
      TRUE ~ NA_character_)) %>%
  select(datetime_hour, day, month, phase, zone, burn_status, north_temp, south_temp, charred_temp, uncharred_temp, ns_avg_temp, cuc_avg_temp, sc_avg_temp, suc_avg_temp, nc_avg_temp, nuc_avg_temp, ns_temp_diff, cuc_temp_diff, sc_temp_diff, suc_temp_diff, nc_temp_diff, nuc_temp_diff, sunrise, sunset, solarNoon)
```
...
### Weather Station Data with Phase
