...
filtered_transun_wx_15min <- transub_wx_15min_r %>%
  mutate(datetime = as.POSIXct(datetime, format = "%Y-%m-%d %H:%M:%S", tz = "MST")) %>%
  filter(datetime >= as.POSIXct("2024-02-02 00:00:00", tz = "MST") & 
         datetime < as.POSIXct("2024-07-01 00:00:00", tz = "MST"))
```
...
full_15min_ns_dtnt <- full_15min_ns %>%
  ...
  mutate(
    phase = case_when(
      ...
      datetime >= sunset ~ "night",
      TRUE ~ NA_character_)) %>%
  select(datetime, day, month, phase, zone, snow_phase, burn_status, north_temp, south_temp, ns_avg_temp, ns_temp_diff, sunrise, sunset, solarNoon)
```
...
full_hourly_ns_dtnt <- full_hourly_ns %>%
  ...
  mutate(
    phase = case_when(
      ...
      TRUE ~ NA_character_)) %>%
  select(datetime_hour, day, month, phase, zone, snow_phase, burn_status, north_temp, south_temp, ns_avg_temp, ns_temp_diff, sunrise, sunset, solarNoon)
```
