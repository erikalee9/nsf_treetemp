...
final_hourly_gb_temps <-final_gb_temps %>%
  ...
  group_by(datetime_hour, zone, burn_status) %>%  # Group by the hourly datetime
...
final_hourly_ns_ub <- final_15min_ns_ub %>%
  ...
  mutate(date = date(datetime_hour), month = month(datetime_hour), hour = hour(datetime_hour)) %>%
  group_by(datetime_hour, date, month, zone, burn_status) %>%
...
final_hourly_ns_b <- final_15min_ns_b %>%
  ...
  mutate(datetime_hour = floor_date(datetime, unit = "hour")) %>%
  mutate(date = date(datetime_hour), month = month(datetime_hour), hour = hour(datetime_hour)) %>%
  group_by(datetime_hour, date, hour, month, zone, burn_status) %>%
  summarize(north_temp = mean(north_temp, na.rm = TRUE), south_temp = mean(south_temp, na.rm = TRUE), avg_temp = mean(avg_temp, na.rm = TRUE), .groups = 'drop')
```
...
write_csv(final_hourly_ns_ub, "nsf/treetemp_data/final_hourly_ns_yb.csv")
```
...
final_hourly_gbdb <- final_15min_gbdb %>%
  ...
  mutate(datetime_hour = floor_date(datetime, unit = "hour")) %>%
  mutate(date = date(datetime_hour), month = month(datetime_hour), hour = hour(datetime_hour)) %>%
  group_by(datetime_hour, date, hour, month, zone, aspect) %>%
...
full_hourly_ns_b <- bind_rows(final_persb_ns_temps, final_transb_ns_temps) %>%
  mutate(datetime = as.POSIXct(datetime, format = "%Y-%m-%d %H:%M:%S")) %>% 
  mutate(datetime_hour = floor_date(datetime, unit = "hour")) %>%
  mutate(day = as.Date(datetime_hour), month = month(datetime_hour), time = format(datetime_hour, "%H:%M:%S")) %>%
  mutate(ns_avg_temp = mean((north_temp + south_temp) / 2)) %>%
  select(datetime, time, day, month, zone, everything()) %>%
  #calculating temp difference for 15-minute intervals
  mutate(temp_diff = south_temp - north_temp) %>%
  select(datetime, day, time, zone, burn_status, north_temp, south_temp, ns_avg_temp, temp_diff)
...
full_hourly_gbdbub <- bind_rows(final_persb_gbdb_temps, final_transb_gbdb_temps) %>%
  full_join(final_15min_gbdb_ub, by = "datetime") %>% # Ensure you're joining on the correct column
